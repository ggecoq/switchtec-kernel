...
static void switchtec_ntb_init_db(struct switchtec_ntb *sndev)
{
	...
	iowrite64(sndev->db_mask | 0x000000000000000FULL << 60, &sndev->mmio_self_dbmsg->idb_mask);
	iowrite64(sndev->db_valid_mask << sndev->db_peer_shift,
		  &sndev->mmio_peer_dbmsg->odb_mask);
...
static int switchtec_ntb_init_db_msg_irq(struct switchtec_ntb *sndev)
{
	...
	sndev->doorbell_irq = sndev->stdev->msix[doorbell_irq].vector;
	sndev->message_irq = sndev->stdev->msix[message_irq].vector;
